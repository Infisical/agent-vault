...
- `services[].name` -- canonical identifier (slug, 3–64 lowercase alphanumeric/hyphen chars). **Optional for `"set"`**: when omitted, the server derives a slug from `host` + `path` (e.g. `api.stripe.com` → `api-stripe-com`, `slack.com/api/apps.connections.*` → `slack-com-api-apps-connections`); supply an explicit `name` to pick a friendlier identifier or to in-place replace an existing service by name. `"delete"` may omit `name` to fall back to host-based resolution: when the host is shared by multiple services the server returns 409 with the candidate names so the caller can retry by `name`.
