...
- `in` declares which surfaces the broker is allowed to scan: subset of `["path", "query", "header", "body", "websocket"]`, defaulting to `["path", "query"]`. `header`, `body`, and `websocket` must be explicit.
...
- `body` scans the HTTP request body. Encoding is content-type-aware: form-urlencoded values are URL-encoded, JSON values are JSON-string-escaped, and other types use raw replacement. Multipart and compressed bodies are skipped. Use for OAuth `client_secret_post` flows.
- `websocket` scans client→server WebSocket text frames. Only unfragmented, uncompressed text frames are scanned. The broker strips `Sec-WebSocket-Extensions` from the upgrade handshake when `websocket` is active to prevent compression. Use for protocols like Discord Gateway where the auth token appears inside a JSON message.
...
- `services[].host` -- single matcher field. Accepts a bare hostname (e.g. `api.stripe.com`), a one-level wildcard (e.g. `*.github.com`), an inline path-scoped form (e.g. `slack.com/api/*`), or an inline port-scoped form (e.g. `localhost:8080/api/*`). The server splits the path and port off the host on ingest. Path globs use `*` as a greedy glob (cross-`/`); `**`, `?`, regex, and bare `*` are rejected. A service with a port matches only traffic to that port; a service without a port matches any port (backward compatible).
...
- `services[].substitutions` -- optional list of URL/header/body/websocket rewrites. Each entry has `key` (UPPER_SNAKE_CASE credential reference), `placeholder` (the exact wire string the broker matches case-sensitively, e.g. `__account_sid__`), and optional `in` (subset of `["path", "query", "header", "body", "websocket"]`; defaults to `["path", "query"]`). Surfaces not in `in` are not scanned. Must be paired with an `auth` change in the same proposal — substitutions cannot be added on an enable/disable-only update. See the URL Substitutions section above.
...
3. **Port-matched beats no-port on tie.** When host tier and literal path-prefix length are equal, a port-scoped service (e.g. `host=localhost:8080`) beats a no-port service (e.g. `host=localhost`) for traffic to that port. A no-port service still serves any other port.
4. **Declaration order on tie.** If two rules tie on the rules above, the rule appearing first in the configured service list wins.
