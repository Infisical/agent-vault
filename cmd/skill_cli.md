...
- `in` declares which surfaces the broker is allowed to scan: subset of `["path", "query", "header", "body", "websocket"]`. Defaults to `["path", "query"]` if omitted. `header`, `body`, and `websocket` must be explicit.
- **Scoping is the security boundary.** The broker only scans surfaces in `in`. Embedding the placeholder anywhere else (a non-declared surface) means the literal string passes through unmodified — there is no way to coerce the broker into substituting somewhere the operator did not authorize.
...
- `body` scans the HTTP request body. Encoding is content-type-aware: `application/x-www-form-urlencoded` values are URL-encoded, `application/json` values are JSON-string-escaped, and all other types use raw replacement. Multipart bodies are skipped. Compressed request bodies (`Content-Encoding` set) are also skipped. Use `body` for OAuth `client_secret_post` flows where the secret is in the POST form body.
- `websocket` scans WebSocket text frames sent from client to server. Only unfragmented, uncompressed text frames are scanned; binary frames, control frames, fragmented messages, and compressed frames pass through unmodified. When `websocket` is in `in`, the broker strips `Sec-WebSocket-Extensions` from the upgrade handshake to prevent compression negotiation. Use `websocket` for protocols like Discord Gateway where the auth token appears inside a JSON message body, not in HTTP headers.
...
- `services[].substitutions` -- optional list of URL/header/body/websocket rewrites. Each entry has `key` (UPPER_SNAKE_CASE credential reference), `placeholder` (the exact wire string the broker matches case-sensitively, e.g. `__account_sid__`), and optional `in` (subset of `["path", "query", "header", "body", "websocket"]`; defaults to `["path", "query"]`). Surfaces not in `in` are not scanned. Must be paired with an `auth` change in the same proposal — substitutions cannot be added on an enable/disable-only update.
