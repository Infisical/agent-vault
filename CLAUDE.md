...
- **Agent skill is the agent-facing contract.** [cmd/skill_cli.md](cmd/skill_cli.md) is embedded into the binary, installed by `vault run` by default (unless `--no-skills` is supplied), and served publicly at `/v1/skills/cli`. It teaches agents how to create proposals when API access is needed.
