...
Agent Vault exposes a transparent HTTP/HTTPS proxy listener on port `14322` — the canonical ingress agents use. Standard HTTP clients that honor `HTTPS_PROXY`/`HTTP_PROXY` (curl, fetch, requests, the Go stdlib, SDKs, CLIs) transparently route through the broker. The listener is plain HTTP, handling both CONNECT (for `https://` upstreams) and absolute-form forward-proxy requests (for `http://` upstreams) on the same port.

<Warning>
Axios-based clients may require an explicit HTTPS proxy agent or adapter; setting `HTTPS_PROXY`/`HTTP_PROXY` alone does not guarantee that Axios will use the broker's CONNECT path. Do not disable Axios proxying unless the replacement transport is known to preserve Agent Vault routing, audit, and credential injection.
</Warning>
