...
Setting `INFISICAL_URL` enables Infisical-backed vaults. When a vault is created with `--credential-store=infisical`, Agent Vault polls Infisical at a per-vault cadence and caches the encrypted secret snapshot in its local DB. Local credential mutations are rejected for those vaults; Infisical is the source of truth. See the [Credential stores](/learn/credential-stores) page for the full mental model.

Supply env vars for exactly one auth method. The Infisical Go SDK does not auto-detect; Agent Vault picks the first complete group in this priority order:
...
GCP IAM and GCP ID Token both read `INFISICAL_GCP_AUTH_IDENTITY_ID`. Setting `INFISICAL_GCP_IAM_SERVICE_ACCOUNT_KEY_FILE_PATH` alongside the identity ID selects GCP IAM; absent the file path Agent Vault selects GCP ID Token (which pulls a JWT from the GCP metadata server at runtime).
