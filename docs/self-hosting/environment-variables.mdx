...
| `AGENT_VAULT_UI_BASE_PATH` | Optional (defaults to empty = domain root) | URL path prefix the UI and API are served under (e.g., `/vault`), for reverse-proxy deployments on a shared domain. The proxy must pass the path through unmodified. Overridden by `--ui-base-path`. See [Serving under a path prefix](/self-hosting/path-prefix). |
