...
| `AGENT_VAULT_NODE_COMPAT` | Optional (defaults to `auto`) | Whether `agent-vault run` injects the Node compatibility preload (`~/.agent-vault/openclaw-compat.js`) into the child via `NODE_OPTIONS=--require=…`. One of: `auto` (detect OpenClaw and inject only for those invocations), `on` (force-inject), `off` (skip). Overridden by `--node-compat`. Host-isolation only — has no effect with `--isolation=container`. |
