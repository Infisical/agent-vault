...
| `AGENT_VAULT_NODE_COMPAT` | Optional (defaults to `auto`) | Whether `agent-vault run` injects the Node compatibility preload (`~/.agent-vault/openclaw-compat.js`) into the child via `NODE_OPTIONS=--require=…`. One of: `auto` (detect OpenClaw and inject only for those invocations), `on` (force-inject), `off` (skip). Overridden by `--node-compat`. Host-isolation only — has no effect with `--isolation=container`. **Requires Node.js ≥ 22.7.0** in the wrapped child (NODE_USE_ENV_PROXY support); the preload fails fast on older runtimes rather than risk silently bypassing the broker for axios calls. |
