...
To change the master password later: stop all instances, update the value in your secret store, and redeploy. The `agent-vault master-password` CLI commands are not available when `DATABASE_URL` is set.
...
<Tip>
  If your current master password is short or guessable, strengthen it before you set `DATABASE_URL`. Run `agent-vault master-password change` while still on SQLite -- this is a single-instance operation and straightforward. After migration, the `master-password` CLI commands are no longer available.
</Tip>
