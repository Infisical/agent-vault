...
```
  ...
  |  1        | |  2        | |  N        |
  | :14321 API| | :14321 API| | :14321 API|
  | :14322    | | :14322    | | :14322    |
  +-----+-----+ +-----+-----+ +-----+-----+
        |             |             |
+-------+-------------+-------------+-------+
|              Load balancer(s)              |
|  L7 (HTTP) for :14321  |  L4 for :14322   |
+------------------------+------------------+
                         |
                      Agents
```
...
To rotate the master password: stop all instances, run `agent-vault master-password change --force`, update the value in your secret store, then restart all instances.
...
<Tip>
  If your master password is weak, you can strengthen it before migrating by running `agent-vault master-password change` while still on SQLite.
</Tip>
