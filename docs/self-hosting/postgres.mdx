...
```
      ...
      |    1     |  |    2     |  |    N     |
      | API+MITM |  | API+MITM |  | API+MITM |
      +----+-----+  +----+-----+  +----+-----+
           |              |              |
   +-------+--------------+--------------+-------+
   |             Load balancer(s)                 |
   |  L7 (HTTP) for API  |  L4 (TCP) for proxy   |
   +----------------------+-----------------------+
                          |
                       Agents
```

Each instance connects to the same Postgres database, shares the same CA root certificate (stored encrypted in the database), and uses advisory locks to prevent conflicting writes.
...
<Tip>
  If your current master password is short or guessable, strengthen it before migrating. Run `agent-vault master-password change` while still on SQLite -- this is a single-instance operation and straightforward. After migration, password changes require stopping all instances and redeploying.
</Tip>
