...
<Tip>
By default, `agent-vault run` also installs a [Claude Code skill](https://docs.anthropic.com/en/docs/claude-code/skills) at `~/.claude/skills/agent-vault-cli/SKILL.md` that teaches Claude Code how to raise proposals when API access is needed. Pass `--no-skills` to skip host-mode skill-file installation or updates. The skill persists across sessions.
</Tip>
