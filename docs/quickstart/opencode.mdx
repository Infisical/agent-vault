...
Launch OpenCode with `vault run`. It wraps OpenCode with a vault-scoped session, pre-configures `HTTPS_PROXY` and CA trust so outbound HTTPS routes through Agent Vault transparently, and installs the Agent Vault skill so OpenCode knows how to discover services and raise proposals. If you haven't logged in yet, you'll be guided through setup automatically.
