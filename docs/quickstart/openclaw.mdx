...
<Tabs>
  <Tab title="Shell">
    ...
    `agent-vault run` automatically configures OpenClaw's managed proxy (`proxy.enabled`) and trusted env proxy for `web_fetch`, sets `OPENCLAW_PROXY_URL`, `HTTPS_PROXY`, `HTTP_PROXY`, and CA trust variables, and by default installs the Agent Vault skill. Pass `--no-skills` to skip only host-mode skill-file installation or updates; OpenClaw proxy configuration remains enabled. No manual OpenClaw configuration needed.
