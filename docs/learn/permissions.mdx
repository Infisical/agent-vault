...
<AccordionGroup>
  <Accordion title="Owner-level vault operations">
    ...
    - **Manage agents**: `agent-vault agent list`, `info`, `revoke`, `rotate`, `rename`, and `set-role` are instance-level operations. Any authenticated user can invite agents; vault admins control per-vault agent access.
