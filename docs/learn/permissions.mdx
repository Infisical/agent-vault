...
| Manage agents (create, list, revoke, rotate, rename, set-role) | Yes | Yes | No |
...
<AccordionGroup>
  <Accordion title="Owner-level vault operations">
    ...
    - **Manage agents**: `agent-vault agent list`, `info`, `delete`, `rotate`, `rename`, and `set-role` are instance-level operations. Any authenticated user can create agents; vault admins control per-vault agent access.
