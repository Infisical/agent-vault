...
- **Name**: A canonical per-vault identifier (slug, 3–64 lowercase alphanumeric/hyphen chars, no leading/trailing hyphen, no consecutive hyphens). **Optional on write** — when omitted, the server slugifies `host` + `path` (e.g. `api.stripe.com` → `api-stripe-com`); supply an explicit name like `stripe`, `slack-bot`, or `internal-billing` for a friendlier identifier. Path-scoped services that share a bare host (e.g. `slack.com/api/*` vs `slack.com/api/apps.connections.*`) end up with distinct slugs automatically, but you can still set `name` explicitly so subsequent operations use the friendlier label.
...
<AccordionGroup>
  <Accordion title="How proposals modify services">
    ...
    - **`set`**: Idempotent upsert keyed by canonical `name` (slug). Adds the service when
      no entry has that name, or replaces it when one does. `name` is optional — when
      omitted, the server slugifies `host` + `path` (e.g. `api.stripe.com` →
      `api-stripe-com`). Supply an explicit `name` when you want a friendlier identifier
      or to in-place replace a specific existing service.
