...
<AccordionGroup>
  <Accordion title="How proposals modify services">
    ...
    - **`set`**: Idempotent upsert keyed by canonical `name` (slug). Adds the service when
      no entry has that name, or replaces it when one does. `name` is required — pick it
      deliberately so the canonical identifier survives renames and rotations cleanly.
