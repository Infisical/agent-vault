...
- **Host**: The single matcher field. Accepts a bare hostname (`api.stripe.com`), a one-level wildcard (`*.github.com`), an inline path-scoped form (`slack.com/api/*`), or an inline port-scoped form (`localhost:8080/api/*`). Reads and writes use the same shape: writes accept any of those forms; reads return the joined inline form so you see exactly what you wrote.
...
The `host` pattern is the single matcher knob. It can carry just a hostname (`api.stripe.com`), a wildcard host (`*.github.com`), an inline path glob (`slack.com/api/*`), or an inline port (`localhost:8080/api/*`).
...
**Port portion** (optional, follows the host with `:`) restricts the rule to traffic on that port. `localhost:8080` matches only traffic to port 8080; omitting the port matches any port (the legacy behavior). Use this when the same host serves different services on different ports.
...
3. **Port-matched beats no-port on tie.** When host tier and literal path-prefix length are equal, a port-scoped rule (`localhost:8080`) beats a no-port rule (`localhost`) for traffic to that port. The no-port rule still serves any other port.
