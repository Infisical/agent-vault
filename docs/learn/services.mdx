...
The `host` pattern is the single matcher knob. It can carry just a hostname (`api.stripe.com`), a wildcard host (`*.github.com`), or an inline path glob (`slack.com/api/*`).
...
<AccordionGroup>
  <Accordion title="How proposals modify services">
    ...
    - **`set`**: Idempotent upsert keyed by canonical `name` (slug). Adds the service when
      ...
      `host` when omitted; a proposal whose `host` matches an existing service adopts that
      service's name (so legacy "rotate credentials" proposals update in place rather than
      create a duplicate).
