...
- **Name**: A canonical per-vault identifier (slug, 3–64 lowercase alphanumeric/hyphen chars). Auto-derived from `host` and `path` when omitted on write.
- **Host**: The target hostname. Supports exact match (e.g. `api.stripe.com`) or one-level wildcard (e.g. `*.github.com`).
- **Path** (optional): A URL path glob (e.g. `/api/*`). Empty matches any path. Lets you attach different credentials to different paths on the same host.
...
<Tip>
  The `/discover` endpoint returns each service's `name`, `host`, `path`, and
  `description`. Agents use this to know which services are already available
  before raising a [proposal](/learn/proposals). When two services share a host
  (e.g. Slack with separate Bot and Connection rules), distinguish them by `name`.
</Tip>
...
services:
  ...
  # Slack uses two credentials at the same host — distinguish by path.
  - name: slack-bot
    host: slack.com
    path: /api/*
    description: Slack Bot User
    auth:
      type: bearer
  ...
  - name: slack-conn
    ...
    description: Slack Socket Mode connections
    auth:
      type: bearer
