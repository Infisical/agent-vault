...
A `Service` is a per-[vault](/learn/vaults) configuration that defines which upstream traffic [agents](/agents/overview) can reach through the proxy and how Agent Vault authenticates to each one. When an agent makes a proxied request, Agent Vault matches the request's host (and optionally URL path) against the vault's services, attaches the configured [credentials](/learn/credentials), and forwards the request. If no service matches, the request is forwarded as plain proxy traffic by default; vault admins can flip a vault into **strict deny mode** (`unmatched_host_policy=deny` in vault settings) to reject unmatched requests with `403` instead.

Each service contains:

- **Name**: A canonical per-vault identifier (slug, 3–64 lowercase alphanumeric/hyphen chars, no leading/trailing hyphen, no consecutive hyphens). Auto-derived from `host` when omitted on write — the host (with any inline path's literal portion) is lower-cased and joined with hyphens, with `.` and `/` becoming `-`. Examples: `api.stripe.com` → `api-stripe-com`; `slack.com/api/*` → `slack-com-api`. When the auto-slug would collide with an existing service's name, the server appends `-2`, `-3`, … so a stale auto-slug never silently overwrites another service.
- **Host**: The target. Accepts a bare hostname (`api.stripe.com`), a one-level wildcard (`*.github.com`), or an inline path-scoped form (`slack.com/api/*`). The server splits the path off the host on ingest; reads return `host` and `path` separately so the matcher rule is inspectable.
- **Path** (read-only on writes; derived from inline `host`): The split URL path glob. Empty matches any path. Lets you attach different credentials to different paths on the same host. Returned on reads so you can see what the matcher will key on.
...
<Tip>
  The `/discover` endpoint returns each service's `name`, `host`, and `path`.
  Agents use this to know which services are already available before raising
  a [proposal](/learn/proposals). When two services share a host (e.g. Slack
  with separate Bot and Connection rules), distinguish them by `name`.
</Tip>
...
Agent Vault matches incoming requests to services on two axes: **host** and (optionally) **path**.

**Host** supports two modes:
...
**Path** is an optional URL glob. Empty matches any path; non-empty must start with `/` and may use `*` as a greedy glob (cross-`/`). `**`, `?`, regex, and a bare `*` are rejected.

### Matching priority

When two services could match a request, the winner is chosen deterministically:
...
- To override a wildcard-host rule for a specific subdomain, **add the exact-host rule** — it takes priority no matter what its path looks like.
- To layer two credentials on the same host, give them **different paths**; the more specific path will win.
- The matcher does not run regex, does not match on method/headers/query/body, and does not bound `*` at path segments. Patterns that tie under rule 2 (e.g. `/api/*/v2` and `/api/*/v3`) fall to declaration order.
...
<Note>
  `vault service remove`, `enable`, and `disable` accept either the canonical service `name` or the `host`. When several path-scoped services share a host (e.g. Slack with `/api/*` and `/api/apps.connections.*`), passing the bare host returns `409 multiple services match host …` with a `candidates` array of `{name, host, path}`. Retry with the specific name shown in `agent-vault vault service list`.
</Note>
...
<AccordionGroup>
  <Accordion title="How proposals modify services">
    ...
    - **`set`**: Idempotent upsert keyed by canonical `name` (slug). Adds the service when
      no entry has that name, or replaces it when one does. `name` is auto-derived from
      `host` when omitted (the server splits any inline path off the host first); a host-only
      proposal that matches an existing service's `(host, path)` adopts the existing name
      (so legacy "rotate credentials" proposals update in place rather than create a duplicate).
    - **`delete`**: Removes the service matching the given `name`. When `name` is omitted,
      `host` is used as a fallback — a unique host match is resolved automatically; a host
      shared by multiple path-scoped services returns `409 multiple services match host …`
      with a `candidates` list, and the proposal must be retried with an explicit `name`.
    ...
    `name`; auto-slug collisions with unrelated existing services are resolved by appending
    `-2`, `-3`, … so a stale auto-slug can't silently overwrite an existing service.
