...
- **Instance roles** (`owner`, `member`) control who can manage users, vaults, and server settings. Owners can see and join any vault, but must explicitly join to access vault contents.
- **Vault roles** (`admin`, `member`) control who can approve [proposals](/learn/proposals), manage agents, and use the proxy within a specific [vault](/learn/vaults).
- **Agents** are scoped to a single vault and cannot access other vaults or perform instance-level operations.
