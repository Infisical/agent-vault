...
- **[Services](/learn/services)**: Definitions of which upstreams (scoped by host and optional URL path, e.g. `api.stripe.com` or `slack.com /api/apps.connections.*`) can be accessed through the vault along with which and how credential(s) must be attached onto each proxy request.
