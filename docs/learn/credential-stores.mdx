...
There is no automatic fail-closed window. `--poll-interval-seconds` (along with the rest of the Infisical config) is set at vault create time only; there is no update path. To shrink the staleness window for an existing vault, delete and recreate it with a smaller value.
...
Setting `INFISICAL_URL` plus one auth-method env-var group enables Infisical-backed vaults at the instance level. See [Environment variables: Infisical credential store](/self-hosting/environment-variables) for the full list of supported auth methods and the detection priority order.
