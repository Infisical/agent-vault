...
- `refresh_params` (optional): additional string form fields sent during refresh. The reserved fields `grant_type`, `refresh_token`, `client_id`, `client_secret`, and `scope` cannot be overridden. Values are never written to logs or validation errors.
