...
- `set`: idempotent upsert (add or replace). Services require `host` + `auth`; the server auto-derives `name` (slug) from `host` when omitted, and credential slots require `key`. To layer two credentials on the same host, embed different inline path globs in `host` (e.g. `slack.com/api/*` vs `slack.com/api/apps.connections.*`).
