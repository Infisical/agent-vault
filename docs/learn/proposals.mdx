...
- `set`: idempotent upsert (add or replace). Services require `host` + `auth`; the server auto-derives `name` (slug) from `host` and `path` when omitted, and credential slots require `key`. To layer two credentials on the same host, give them different `path` globs (e.g. `/api/*` vs `/api/apps.connections.*`).
