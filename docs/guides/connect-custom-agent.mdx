...
If your agent wasn't launched via `vault run` (e.g. an agent created via `agent create`, a Docker container, a sandboxed runtime), configure the proxy yourself. The MITM listener is TLS-encrypted, so the proxy URL uses the `https://` scheme and needs the root CA trusted. Set `HTTPS_PROXY` and `HTTP_PROXY` to the same URL — the listener handles `CONNECT` (for `https://` upstreams) and absolute-form forward-proxy requests (for `http://` upstreams) on the same port.
