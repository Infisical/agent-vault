...
```json Response
{
  ...
  "services": [
    ...
    { "name": "github", "host": "api.github.com", "description": "GitHub API" }
  ],
  "available_credentials": ["STRIPE_KEY", "GITHUB_TOKEN"]
}
```

- **`services`** lists the services configured in the vault, scoped by host and optional path. Each entry has a canonical `name` (slug). Requests not matching any service go direct.
