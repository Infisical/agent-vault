...
### Proxy CA trust for curl

`curl` verifies the HTTPS proxy's TLS certificate separately from upstream certificates, and has no env var for it. `agent-vault run` handles this by writing a `~/.curlrc` with `proxy-cacert` pointing at the MITM CA, so child processes that invoke `curl` work without extra flags.

If `.curlrc` doesn't work in your environment, pass `--proxy-cacert ~/.agent-vault/mitm-ca.pem` per command, or install the CA into the system trust store (`update-ca-certificates` on Debian/Ubuntu/Alpine, `update-ca-trust extract` on RHEL/Fedora).
