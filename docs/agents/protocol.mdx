...
```json Response
{
  "vault": "my-vault",
  "services": [
    { "name": "stripe", "host": "api.stripe.com", "description": "Stripe API" },
    { "name": "github", "host": "*.github.com", "description": "GitHub API" },
    { "name": "slack-bot", "host": "slack.com", "path": "/api/*", "description": "Slack Bot User" },
    { "name": "slack-conn", "host": "slack.com", "path": "/api/apps.connections.*", "description": "Slack Socket Mode" }
  ],
  "available_credentials": ["GITHUB_TOKEN", "SLACK_BOT_TOKEN", "SLACK_CONNECTION_TOKEN", "STRIPE_KEY"]
}
```

- **`services`** lists the services the agent can reach through Agent Vault (each entry has `name`, `host`, optional `path`, and `description`). Requests to any other host go direct. When two services share a host (e.g. Slack here), distinguish them by `name` in subsequent operations.
