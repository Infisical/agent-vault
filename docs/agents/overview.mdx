...
By default, agents are created with the `member` instance role. Use `--role` to set a different instance-level role:

```bash
# Create an agent as an instance owner
agent-vault agent create my-agent --role owner

# Create an agent with no instance-scoped capabilities — useful when the
# agent should only operate within a specific vault at proxy role.
agent-vault agent create my-agent --role no-access --vault payments:proxy
```
