...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    **Environment variables:**
    ...
    | `INFISICAL_URL` | Base URL of an Infisical instance. When set, the server constructs a machine-identity client from one of the `INFISICAL_*` env-var groups below and enables `--credential-store=infisical` on `vault create`. See [Environment variables: Infisical credential store](/self-hosting/environment-variables) for the auth-method detection priority order. |
    ...
    | `INFISICAL_KUBERNETES_IDENTITY_ID` | Machine identity ID for Kubernetes-mounted service-account tokens. |
    | `INFISICAL_KUBERNETES_SERVICE_ACCOUNT_TOKEN_PATH` | Override path to the projected service-account token (default: SDK default). |
    | `INFISICAL_AWS_IAM_AUTH_IDENTITY_ID` | Machine identity ID for AWS IAM auth. |
    | `INFISICAL_GCP_AUTH_IDENTITY_ID` | Machine identity ID for GCP IAM or GCP ID Token auth (shared by both flows). |
    | `INFISICAL_GCP_IAM_SERVICE_ACCOUNT_KEY_FILE_PATH` | Path to the GCP service-account JSON key. Its presence selects GCP IAM over GCP ID Token. |
    | `INFISICAL_LDAP_AUTH_IDENTITY_ID` | Machine identity ID for LDAP auth. |
...
<AccordionGroup>
  <Accordion title="agent-vault vault create">
    ...
    | `--infisical-recursive` | `false` | Pull secrets recursively below the path. |
    | `--poll-interval-seconds` | `60` | How often the broker refreshes the cached secrets. Minimum 10s. |
  </Accordion>
