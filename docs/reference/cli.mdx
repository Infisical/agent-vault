...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    | `--ui-base-path` | `""` (domain root) | URL path prefix to serve the UI **and** API under, e.g. `/vault` — for deployments behind a reverse proxy on a shared domain. The proxy must pass the path through unmodified. `GET /health` stays available at the root, and `GET /` redirects to the prefix. Also respects `AGENT_VAULT_UI_BASE_PATH`; the flag takes precedence. See [Serving under a path prefix](/self-hosting/path-prefix). |
    ...
    **Environment variables:**
    ...
    | `AGENT_VAULT_UI_BASE_PATH` | URL path prefix to serve the UI and API under (default empty = domain root). The `--ui-base-path` flag takes precedence. See [Serving under a path prefix](/self-hosting/path-prefix). |
