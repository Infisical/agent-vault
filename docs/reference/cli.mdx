...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    **Environment variables:**
    ...
    | `AGENT_VAULT_NOTIFY_WEBHOOK_URL` | Outbound webhook URL for proposal notifications. If unset, webhook notifications are disabled. |
    | `AGENT_VAULT_NOTIFY_FORMAT` | Webhook payload format: `generic` (default, JSON event payload) or `slack` (`{"text": ...}`, also Discord-compatible) |
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault owner notify webhook test">
    ```bash
    agent-vault owner notify webhook test [flags]
    ```

    Send a synthetic `proposal.created` event to verify webhook configuration. Owner only.
