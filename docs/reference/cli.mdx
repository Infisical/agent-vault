...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault service add">
    ...
    | `--name` | _derived_ | Service name (slug, 3–64 lowercase alphanumeric/hyphen chars). Optional — when omitted, the server slugifies `--host` + the inline path (e.g. `api.stripe.com` → `api-stripe-com`). Supply explicitly for a friendlier identifier or to in-place replace a specific existing service. |
