...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault service add">
    ...
    | `--name` | _auto_ | Service name (slug, 3–64 lowercase alphanumeric/hyphen chars). Auto-derived from `--host` and `--path` when omitted. |
    | `--host` | | Target service host (e.g. `api.stripe.com`). For convenience, a value with a `/` like `slack.com/api/*` is split into host + path. |
    | `--path` | | Optional URL path glob (e.g. `/api/*`). Empty matches any path. |
    ...
    To layer two credentials on the same host, give them different `--path` globs (e.g. Slack with `/api/*` for the Bot token and `/api/apps.connections.*` for Socket Mode). The matcher resolves overlapping rules deterministically — see [Matching priority](/learn/services#matching-priority).
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault proposal create">
    ...
    | `--name` | _auto_ | Service name (slug). Auto-derived from `--host` and `--path` when omitted. |
    | `--host` | | Target service host (e.g. `api.stripe.com`). Inline form `slack.com/api/*` is split into host + path. |
    | `--path` | | Optional URL path glob (e.g. `/api/*`). Empty matches any path. |
    | `--description` | | Service description |
