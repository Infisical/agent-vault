...
<AccordionGroup>
  ...
  <Accordion title="agent-vault run (alias: agent-vault vault run)">
    ...
    | `--share-agent-dir` | `false` | Bind-mount the host state dir for the selected command (`~/.claude`, `~/.cursor`, `~/.codex`, `~/.hermes`, `~/.opencode`) into `/home/claude/<agent-dir>` so the sandbox reuses your host login (`--sandbox=container` only). On Linux the container's `claude` user is remapped to your host uid/gid so writes land owned by you. Mutually exclusive with `--home-volume-shared`. |
  </Accordion>
