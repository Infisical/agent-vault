...
<AccordionGroup>
  <Accordion title="agent-vault server">
    ...
    **Environment variables:**
    ...
    | `AGENT_VAULT_ADDR` | Externally-reachable base URL (e.g. `https://agent-vault.example.com`). Used for links in emails, invites, and discovery, and the hostname is added as a SubjectAltName on every MITM leaf cert so clients that TLS-verify against the proxy's own hostname succeed without a shim. Falls back to `https://<FLY_APP_NAME>.fly.dev` on Fly.io, then `http://{host}:{port}`. The **Connect Your Agent** modal pre-fills the agent address from this value (or the Fly-derived fallback); on a non-Fly deployment without `AGENT_VAULT_ADDR` set, the modal renders a literal `<AGENT_VAULT_ADDR>` placeholder rather than the bind-addr fallback, which is rarely reachable from a remote agent. |
