...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault service add">
    ...
    | `--auth-type` | | Auth type: `bearer`, `basic`, `api-key`, `custom`, `oauth`, `passthrough` |
    ...
    | `--oauth-client-secret-key` | | Credential key for OAuth client secret |
    | `--oauth-refresh-token-key` | | Credential key for OAuth refresh token |
    | `--oauth-token-endpoint` | | OAuth token endpoint URL |
    | `--oauth-scope` | | OAuth scope to request on refresh (repeatable) |
    | `--disabled` | `false` | Create the service in a disabled state (proxy traffic returns 403 until enabled) |

    For `oauth`, Agent Vault stores the refresh token and client secret in the vault, refreshes access tokens at request time, and injects `Authorization: Bearer <access_token>`. See [OAuth services](/learn/oauth-services).
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault proposal create">
    ...
    | `--auth-type` | | Auth type: `bearer`, `basic`, `api-key`, `oauth`, or `passthrough` |
    | `--token-key` | | Credential key for bearer auth |
    | `--username-key` | | Credential key for basic auth username |
    | `--password-key` | | Credential key for basic auth password |
    | `--api-key-key` | | Credential key for api-key auth |
