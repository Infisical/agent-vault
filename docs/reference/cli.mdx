...
<AccordionGroup>
  ...
  <Accordion title="agent-vault run (alias: agent-vault vault run)">
    ...
    **Agent mode (containerized / unattended deployments).** When `AGENT_VAULT_TOKEN` and `AGENT_VAULT_ADDR` are pre-set on the environment, `vault run` skips the admin-session login and uses the env-supplied token as the credential — `--ttl` is rejected in this mode since the token's lifetime is fixed at mint time. The token is validated against the broker once at startup so bad/expired tokens fail fast with a clear error rather than producing 401s on every proxied call. See [Deploy your agent in a container](/guides/deploy-agent-container).
    ...
    | `--node-compat` | `auto` | Inject the Node compatibility preload (`~/.agent-vault/openclaw-compat.js`) into the child's `NODE_OPTIONS=--require=…`. `auto` (default) detects OpenClaw — either as the bare `openclaw` binary or as `node`/`nodejs` invoking an `/openclaw/`-pathed `.js` entrypoint — and injects only for those invocations. `on` forces injection regardless of binary. `off` skips injection (escape hatch for operators who patch axios themselves). Also read from `AGENT_VAULT_NODE_COMPAT`. Host-isolation only — has no effect with `--isolation=container`. **Requires Node.js ≥ 22.7.0** in the wrapped child process (NODE_USE_ENV_PROXY support); the preload fails loudly on older runtimes rather than risk silently bypassing the broker for axios calls. |
