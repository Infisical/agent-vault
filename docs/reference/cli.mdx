...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault service add">
    ...
    | `--host` | | Target service host. Accepts a bare hostname (`api.stripe.com`), a one-level wildcard (`*.github.com`), an inline path-scoped form (`slack.com/api/*`), or an inline port-scoped form (`localhost:8080/api/*`). The server splits the path and port off the host on ingest. A service with a port matches only traffic to that port; a service without a port matches any port. |
    ...
    To layer two credentials on the same host, give them different inline paths or ports in `--host` (e.g. Slack with `slack.com/api/*` for the Bot token and `slack.com/api/apps.connections.*` for Socket Mode; or `localhost:8080` and `localhost:9090` for two local apps). The matcher resolves overlapping rules deterministically — see [Matching priority](/learn/services#matching-priority).
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault proposal create">
    ...
    | `--host` | | Target service host. Accepts a bare hostname (`api.stripe.com`), a one-level wildcard (`*.github.com`), an inline path-scoped form (`slack.com/api/*`), or an inline port-scoped form (`localhost:8080/api/*`). |
