...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault service add">
    ...
    | `--name` | _auto_ | Service name (slug, 3–64 lowercase alphanumeric/hyphen chars). Auto-derived from `--host` when omitted. |
...
<AccordionGroup>
  ...
  <Accordion title="agent-vault vault proposal create">
    ...
    | `--name` | _auto_ | Service name (slug). Auto-derived from `--host` when omitted. |
