...
// TestInject_PortMatch pins that a service with Port set matches only
// traffic on that port. A service with no Port matches any port.
	...
	f.setServices(t, "v1", []broker.Service{
		{
			...
			Host: "api.example.com",
			Port: "8080",
			Auth: broker.Auth{Type: "bearer", Token: "TOK_8080"},
		},
		{
			Name: "port-9090",
			Host: "api.example.com",
			Port: "9090",
			Auth: broker.Auth{Type: "bearer", Token: "TOK_9090"},
		},
	})
	f.setCred(t, key32, "v1", "TOK_8080", "secret-8080")
	f.setCred(t, key32, "v1", "TOK_9090", "secret-9090")

	p := NewStoreCredentialProvider(f, key32)

	// Port 8080 → matches port-8080 service
	res, err := p.Inject(context.Background(), "v1", "api.example.com:8080", "/")
	if err != nil {
		t.Fatalf("port 8080: unexpected err: %v", err)
	}
	if res.Headers["Authorization"] != "Bearer secret-8080" {
		t.Fatalf("port 8080: got Authorization=%q", res.Headers["Authorization"])
	}
	if res.MatchedName != "port-8080" {
		t.Fatalf("port 8080: got MatchedName=%q want port-8080", res.MatchedName)
	}
	if res.MatchedPort != "8080" {
		t.Fatalf("port 8080: got MatchedPort=%q want 8080", res.MatchedPort)
	}

	// Port 9090 → matches port-9090 service
	res, err = p.Inject(context.Background(), "v1", "api.example.com:9090", "/")
	if err != nil {
		t.Fatalf("port 9090: unexpected err: %v", err)
	}
	if res.Headers["Authorization"] != "Bearer secret-9090" {
		t.Fatalf("port 9090: got Authorization=%q", res.Headers["Authorization"])
	}
	if res.MatchedName != "port-9090" {
		t.Fatalf("port 9090: got MatchedName=%q want port-9090", res.MatchedName)
	}
}

// TestInject_PortServiceWithFallback pins that a no-port service catches
// traffic when no port-matched service exists.
func TestInject_PortServiceWithFallback(t *testing.T) {
	key32 := make32(0x91)
	f := newFakeCredStore()
	f.setServices(t, "v1", []broker.Service{
		{
		...
		},
		{
			Name: "any-port",
			Host: "api.example.com",
			Auth: broker.Auth{Type: "bearer", Token: "TOK_ANY"},
		},
	})
	f.setCred(t, key32, "v1", "TOK_8080", "secret-8080")
	f.setCred(t, key32, "v1", "TOK_ANY", "secret-any")

	p := NewStoreCredentialProvider(f, key32)

	// Port 8080 → matches port-8080 (port-matched wins)
	res, err := p.Inject(context.Background(), "v1", "api.example.com:8080", "/")
	if err != nil {
		t.Fatalf("port 8080: unexpected err: %v", err)
	}
	...
	// Port 9090 → matches any-port (no port-matched service)
	res, err = p.Inject(context.Background(), "v1", "api.example.com:9090", "/")
	if err != nil {
		t.Fatalf("port 9090: unexpected err: %v", err)
	}
	if res.MatchedName != "any-port" {
		t.Fatalf("port 9090: got MatchedName=%q want any-port", res.MatchedName)
	}
}
