...
// This Syncer is a deliberate near-copy of internal/infisical/sync.go. Before
// cloning it for a third credential store, read the design note at the top of
// auth.go — prefer extracting a shared engine over a third copy.
...
// SyncerStore is the narrow store surface the syncer needs.
...
// (which can embed VAULT_ADDR + upstream rejection bodies) goes to logs.
const syncFailedPublicMessage = "HashiCorp Vault sync failed. See server logs for details."

var (
	// ErrSyncerDisabled: no Fetcher (e.g. VAULT_ADDR unset). → 503.
	ErrSyncerDisabled = errors.New("hashicorp: syncer disabled (no client)")
	// ErrNotExternal: vault is not HashiCorp-backed. → 400.
	ErrNotExternal = errors.New("hashicorp: vault has no hashicorp credential store")
	// ErrSyncInFlight: another refresh is running for this vault. → 409.
	ErrSyncInFlight = errors.New("hashicorp: sync already in flight for this vault")
	// ErrInvalidKey: upstream secret key violates broker.CredentialKeyPattern.
	ErrInvalidKey = errors.New("hashicorp: upstream secret key does not match required pattern")
)

// Syncer pulls HashiCorp Vault secrets into the local credentials table at each
...
// rest of the server stays up when HashiCorp Vault isn't configured.
...
func (s *Syncer) Run(ctx context.Context) {
	if s.fetcher == nil {
		s.logger.Info("hashicorp syncer disabled (no client)")
		return
	}
	s.logger.Info("hashicorp syncer started", slog.Duration("tick", tickInterval))
	...
	for {
		select {
		case <-ctx.Done():
			s.logger.Info("hashicorp syncer stopped")
			return
		case <-ticker.C:
			s.tick(ctx)
		}
...
// RefreshOnce runs a single synchronous refresh, reusing the periodic syncer's
// in-flight guard. On failure refresh updates the row's health and returns the
// error so the caller can map it to an HTTP status.
...
func (s *Syncer) refresh(ctx context.Context, cs store.VaultCredentialStore) error {
	...
	s.logger.Info("hashicorp sync ok",
		slog.String("vault_id", cs.VaultID),
...
func EncryptSecrets(secs []Secret, dek []byte) ([]store.EncryptedKV, error) {
	...
	for _, sec := range secs {
		if sec.Key == "" {
			return nil, errors.New("hashicorp returned an empty secret key")
		}
...
func (s *Syncer) recordFailure(ctx context.Context, vaultID string, err error) {
	...
	s.logger.Warn("hashicorp sync failed",
