...
- **Pluggable Credential Stores**: Back a vault with an external system like [Infisical](https://infisical.com) or [HashiCorp Vault](https://developer.hashicorp.com/vault) instead of the local encrypted store. Set `INFISICAL_URL` and create vaults with `--credential-store=infisical`, or set `VAULT_ADDR` and use `--credential-store=hashicorp`. See [docs/learn/credential-stores](docs/learn/credential-stores.mdx).
